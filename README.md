...
> 文档基线：2026-09-02
> 当前阶段边界：N40 Route、N41 Sequence、N42 Stage、N43 七工作模式、N50 Player、N51 Gal Settings、N52、N60 与 N61 Engineering 出口已通过。产品负责人已把窄范围授权扩至 N62；[N62-E1 审计 #282](docs/282-n62-e1-additional-content-entry-audit.md)完成正式 Player 附加入口、四类 Catalog/Meta 摘要和返回身份自动化，但本机管理员安全策略阻断 production-browser，故 E1 尚未关闭，后续切片不得抢跑。N21/N23 真人仍为 0/1、0/2，并延后到功能与整体 UI 就绪后执行。全部 Product Acceptance、N70 Engineering、Android 实体包、M1 与发布继续阻断。
...
| [N60-E5 当前场景调试启动审计](docs/272-n60-e5-scene-debugger-start-audit.md) | Scene Fresh Run 产品缺口、真实预期/实际差异、最小修正与总出口接续点 |
...
| [N61-E4 CJK/Ruby/字体回退审计](docs/277-n61-e4-cjk-ruby-font-fallback-audit.md) | 语义 Ruby、严格 CJK 排版、项目字体失败恢复及真实桌面差异 |
| [N61-E5 语言媒体与 Voice Asset 映射审计](docs/278-n61-e5-localized-media-voice-mapping-audit.md) | 语言视觉变体、stable-text-ID Voice、播放中即时切换与缺失资源可见回退 |
| [N61-E6 Production 语言媒体与配音生产审计](docs/279-n61-e6-localized-media-production-audit.md) | 真实资源导入、稳定文本/基础媒体绑定、状态、缺失恢复、保存重开与移动卡片 |
| [N61-E7 本地化与配音 Engineering 出口审计](docs/280-n61-e7-localization-engineering-exit-audit.md) | 同一 Canonical/Blob 跨 Editor、Compiler、Player 交付，390×844 CJK 证据与 N61 出口结论 |
| [N61→N62 跨电脑开发交接](docs/281-n61-to-n62-cross-device-handoff.md) | 远端恢复点、真实代码能力、N62-E1 用户路径、测试合同、授权边界与换机命令 |
| [N62-E1 自动附加内容入口实现审计](docs/282-n62-e1-additional-content-entry-audit.md) | 正式 Player 四类自动摘要、Effect 时点纠偏、返回身份、浏览器阻断与下一接续顺序 |
...
| [N50-E5 Player Web 宿主生命周期审计](docs/217-n50-e5-player-host-lifecycle-audit.md) | 可见性暂停/恢复、媒体冻结、卸载释放、重挂 fresh Core 与真实浏览器证据 |
| [N50-E6 Player 宿主嵌入 API 审计](docs/218-n50-e6-player-embed-api-audit.md) | 版本化 mount/update/suspend/unmount、独立 embed 页与开发/冷生产浏览器纠偏 |
| [N50 Player Engineering 出口复审](docs/219-n50-engineering-exit-reaudit.md) | E1–E6 完成度、N50/N52 范围冲突、三宿主与产品门 fail-closed 判定 |
| [N50–N52 范围消歧审计](docs/220-n50-n52-scope-reconciliation.md) | Player Shell、Settings、Save/History/Auto/Skip/Back 的唯一节点归属与 N50 出口重判 |
| [N50→N51 治理检查点](docs/221-n50-n51-governance-checkpoint.md) | RA-N21-010 有界准入、N52 越界反例和 N51-E1 typed settings 冻结起点 |
...
| [N42-E9 三次贝塞尔角色运动路径审计](docs/199-n42-e9-bezier-character-path-audit.md) | 图形控制点、单一 Canonical Move、Compiler/Runtime Host、Canvas/DOM、保存重开和真实差异测试 |
