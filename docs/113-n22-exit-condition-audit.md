...
- 原 `RA-N21-001` 已由产品顺序纠正决定关闭；`RA-N21-002` 只授权 N23 可运行纵向切片工程实现，仍阻断 N21/N23 产品验收、M1 Stable 和 Public Release；
...
| N23 | 工程实现中 | `RA-N21-002` 允许先建立可运行纵向切片；产品验收仍等待真人门关闭 |
...
1. 先按 [`可运行流程审计与验收顺序纠正`](115-playable-flow-order-correction.md)完成 N23-E1/E2，使空工程创作、保存重开和试玩到结局可执行；
2. 再按 [`N21-HV-01` 执行包](114-n21-human-validation-execution-kit.md)安排真实非程序用户执行完整创作与试玩任务；
3. 将开始/结束时间、求助、阻塞点、输入设备、工程快照、保存关闭重开和到达结局结果写回 N21 证据；
4. 关闭 `RA-N21-002`，完成 N21 Product Acceptance；
5. 补齐五分钟内容量与两名独立验收者后，才允许 N23 Product Acceptance。
