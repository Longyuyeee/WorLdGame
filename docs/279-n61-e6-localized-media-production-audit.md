...
首次精确 head CI 预期全绿，实际 run `33604602206` / job `100165624881` 在 N51 出口审计失败：状态文档把机器所需的独立 token `N51 Engineering 已关闭` 合并成了“`N51、N52、N60 Engineering 已关闭`”。功能回归在此前均已通过；本轮未改测试或门限，只恢复三个节点各自可机器追踪的明确状态。本地 N51 出口审计随即从 FAIL 恢复为 PASS，修正 head `2030062` 的 Windows / Node 22 完整门 run `33605157687` / job `100167349667` 用时 `13m45s` 成功，关闭该文档差异。
