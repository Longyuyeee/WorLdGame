...
| CL-04 | Narrative VM 可确定回滚、前进、快进与 Barrier | 进行中（契约已冻结；[Spike 01](68-cl04-vm-kernel-spike-01.md)–[12](79-cl04-vm-generated-web-worker-conformance-spike-12.md)） | Runtime Owner | Architecture + QA | 三端 State Hash 0 差异；VM-01–VM-15；10k 生成序列；不可重放副作用必须显式 Barrier | `cd95acb`：Node/Web Worker 的 10,000 seeds / 20,000 replays 摘要与失败种子零差异，并回归 28 条宿主记录；仍无完整 Effect/Barrier/Meta/Skip 矩阵、Windows/Android 壳和独立审阅 | Spike 13 扩展 Effect 重复/乱序/取消、Barrier、Meta 与 Skip 跨宿主逐记录矩阵 |
