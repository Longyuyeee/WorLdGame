...
| CL-04 | Narrative VM 可确定回滚、前进、快进与 Barrier | 进行中（契约已冻结；[Spike 01](68-cl04-vm-kernel-spike-01.md)–[05](72-cl04-vm-effect-barrier-spike-05.md)） | Runtime Owner | Architecture + QA | 三端 State Hash 0 差异；VM-01–VM-15；10k 生成序列；不可重放副作用必须显式 Barrier | `40b575a`：确定 Effect token/hash、严格完成/取消、Scheduler cancellation directive、pure/reversible ledger 与 Barrier 阻断；仅 Node 受控宿主，真实 Scheduler/Save/独立审阅待办 | Spike 06 冻结 Runtime Save envelope、完整性、版本拒绝与非破坏迁移，并执行 VM-11/12 |
