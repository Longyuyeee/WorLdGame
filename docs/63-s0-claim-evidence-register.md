...
| CL-04 | Narrative VM 可确定回滚、前进、快进与 Barrier | 进行中（契约已冻结；[Spike 01](68-cl04-vm-kernel-spike-01.md)–[08](75-cl04-vm-meta-progress-spike-08.md)） | Runtime Owner | Architecture + QA | 三端 State Hash 0 差异；VM-01–VM-15；10k 生成序列；不可重放副作用必须显式 Barrier | `1c3bf1e`：VM-13 已读/CG/结局 G-Set、独立 Meta Hash、内容寻址 Save 引用及 Back/旧 Load 不回退；仅 Node 内存快照，无存储事务/Skip 文本映射/三宿主/独立审阅 | Spike 09 执行 VM-14/15 与至少 10k 生成序列，固化失败种子 |
