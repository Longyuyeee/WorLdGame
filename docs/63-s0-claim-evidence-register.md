...
| CL-03 | Windows 壳选型满足文件、更新、内存和隔离 | 进行中（[证据契约](65-cl03-windows-shell-evidence-contract.md)；[Spike 01](82-cl03-windows-shell-spike-01.md)–[04](85-cl03-windows-shell-spike-04.md)） | Windows Platform Owner | Architecture + Security | WS-01–WS-18；WIN-L 预算；安全/数据/更新硬门；通过者再评分 | `13fb28b`：双壳真实双 PID 的 held、正常释放、强杀后到期接管、token 递增和旧 token 写拒绝成立；磁盘租约仍非线性化 CAS；全仓被既有 VM-14 5 秒超时阻断 | 用 Windows 原生锁/CAS 证明同时接管竞争，再做真实 renderer/main WAL 强杀；WS-06/07 均不得通过 |
