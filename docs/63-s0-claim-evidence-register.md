...
| CL-03 | Windows 壳选型满足文件、更新、内存和隔离 | 进行中（[证据契约](65-cl03-windows-shell-evidence-contract.md)；[Spike 01](82-cl03-windows-shell-spike-01.md)–[05](86-cl03-windows-shell-spike-05.md)） | Windows Platform Owner | Architecture + Security | WS-01–WS-18；WIN-L 预算；安全/数据/更新硬门；通过者再评分 | `5c003e3`：双壳各 3 轮 × 8 PID 同步竞争均恰好一赢家、token 单次推进；正常释放/强杀到期接管/旧 token 拒绝保持成立；kill-during-guard 未解；全仓仍被既有 VM-14 超时阻断 | 设计可审计 CAS owner/nonce 与强杀恢复，再做时钟/损坏矩阵和真实 renderer/main WAL 强杀；WS-06/07 均不得通过 |
