...
| CL-03 | Windows 壳选型满足文件、更新、内存和隔离 | 进行中（[证据契约](65-cl03-windows-shell-evidence-contract.md)；[Spike 01](82-cl03-windows-shell-spike-01.md)；[Spike 02](83-cl03-windows-shell-spike-02.md)） | Windows Platform Owner | Architecture + Security | WS-01–WS-18；WIN-L 预算；安全/数据/更新硬门；通过者再评分 | `b341d15`：Electron/Tauri 同源执行 app-private ProjectFileStore；7 个 WAL 边界恢复为 4 旧/3 新/0 损坏；备份、单进程 fencing、路径穿越与 0/2/64 退出成立；非 WIN-L、非真实强杀 | 接入不暴露绝对路径的系统目录 grant，验证 junction/symlink、第二进程、真实强杀、脏关闭和目录耐久；不得选型 |
