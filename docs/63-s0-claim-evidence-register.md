...
| CL-04 | Narrative VM 可确定回滚、前进、快进与 Barrier | 进行中（契约已冻结；[Spike 01](68-cl04-vm-kernel-spike-01.md)–[04](71-cl04-vm-history-spike-04.md)） | Runtime Owner | Architecture + QA | 三端 State Hash 0 差异；VM-01–VM-15；10k 生成序列；不可重放副作用必须显式 Barrier | `6d8f61d`：独立 Runtime Session、完整边界 checkpoint、VM-04/05 Back/Forward/Fork、原子截断与输入 tombstone；仅 Node 宿主，replay/Effect/Save/独立审阅待办 | Spike 05 冻结 Effect intent、await/cancellation 与 Barrier，并执行 VM-06–08 |
