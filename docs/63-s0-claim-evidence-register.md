...
| CL-03 | Windows 壳选型满足文件、更新、内存和隔离 | 进行中（[证据契约](65-cl03-windows-shell-evidence-contract.md)；[Spike 01](82-cl03-windows-shell-spike-01.md)；[Spike 02](83-cl03-windows-shell-spike-02.md)；[Spike 03](84-cl03-windows-shell-spike-03.md)） | Windows Platform Owner | Architecture + Security | WS-01–WS-18；WIN-L 预算；安全/数据/更新硬门；通过者再评分 | `66c3b47`：双壳原生 grant 不向 renderer 暴露绝对路径；卷根/重解析根拒绝；真实子 junction 双 release 宿主拒绝；63/419、Rust 7/7；仍有 TOCTOU，非最终 picker | 建立外部双 PID 锁与 stale-owner 接管，再做真实 renderer/main WAL 强杀；handle-relative reparse 防护未完成前 WS-07 不通过 |
