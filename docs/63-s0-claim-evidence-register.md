...
| CL-04 | Narrative VM 可确定回滚、前进、快进与 Barrier | 进行中（契约已冻结；[Spike 01](68-cl04-vm-kernel-spike-01.md)–[07](74-cl04-vm-skip-auto-spike-07.md)） | Runtime Owner | Architecture + QA | 三端 State Hash 0 差异；VM-01–VM-15；10k 生成序列；不可重放副作用必须显式 Barrier | `f44b01f`：VM-09/10 Normal/5/10/20/40/Instant 同终点 Hash、Skip Read 未读边界、Auto 外部计时、Choice/Barrier/Effect/资源/Stop Point/错误停止；仅 Node State 调度，真实时钟/History 集成/三宿主/独立审阅待办 | Spike 08 冻结独立 Meta Progress 单调合并与 Hash，并执行 VM-13 |
