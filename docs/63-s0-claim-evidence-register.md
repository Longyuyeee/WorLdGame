...
| CL-04 | Narrative VM 可确定回滚、前进、快进与 Barrier | 进行中（契约已冻结；[Spike 01](68-cl04-vm-kernel-spike-01.md)–[10](77-cl04-vm-web-worker-conformance-spike-10.md)） | Runtime Owner | Architecture + QA | 三端 State Hash 0 差异；VM-01–VM-15；10k 生成序列；不可重放副作用必须显式 Barrier | `36f3d74`：Node 与真实 Web Worker 的 12 动作基础 Trace 逐记录零差异；仍无 History/Save 跨宿主、Windows/Android 壳和独立审阅 | Spike 11 把 History/Scheduler 与 Runtime Save Corpus 纳入 Node/Web Worker Trace |
