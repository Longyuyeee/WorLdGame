...
| CL-03 | Windows 壳选型满足文件、更新、内存和隔离 | 进行中（[证据契约](65-cl03-windows-shell-evidence-contract.md)；[探索 Spike 01](82-cl03-windows-shell-spike-01.md)） | Windows Platform Owner | Architecture + Security | WS-01–WS-18；WIN-L 预算；安全/数据/更新硬门；通过者再评分 | `7d5e606`：Electron/Tauri 已在开发机用同源 Web 自执行 CL-04 Bundle 且结果一致；最小桥接和基础拒绝通过；非 WIN-L，仅为探索证据 | 扩展同切片到系统目录、ProjectFileStore/WAL/恢复/单写者，再准备安装更新与 WIN-L；不得选型 |
