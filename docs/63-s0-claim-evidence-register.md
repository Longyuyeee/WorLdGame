...
| CL-04 | Narrative VM 可确定回滚、前进、快进与 Barrier | 进行中（契约已冻结；[Spike 01](68-cl04-vm-kernel-spike-01.md)–[13](80-cl04-vm-effect-barrier-meta-conformance-spike-13.md)） | Runtime Owner | Architecture + QA | 三端 State Hash 0 差异；VM-01–VM-15；10k 生成序列；不可重放副作用必须显式 Barrier | `9f56cff`：Node/Web Worker 的 22 条 Effect/Barrier/Meta/Skip 记录及累计 50 条宿主记录零差异；10k 语料继续通过 | Spike 14 冻结平台壳可执行 Conformance Bundle 与机器可读差异报告 |
