...
| CL-04 | Narrative VM 可确定回滚、前进、快进与 Barrier | 进行中（契约已冻结；[Spike 01](68-cl04-vm-kernel-spike-01.md) + [Spike 02](69-cl04-vm-kernel-spike-02.md)） | Runtime Owner | Architecture + QA | 三端 State Hash 0 差异；VM-01–VM-15；10k 生成序列；不可重放副作用必须显式 Barrier | `fe11324`：十个纯 Opcode、64 层 LIFO、固定 PRNG/逻辑 tick、VM-01 与 VM-02/03 部分 Golden；仅 Node 宿主，正式 Save 与独立审阅待办 | Spike 03 冻结并实现 Choice 与版本化外部输入幂等边界；编码前审计 Save/History 顺序 |
