...
> 建立日期：2026-08-12
> 状态：技术风险证据登记表；不再单独决定产品开发顺序
> 规则：状态只由可复现证据改变；Owner 是执行责任角色，Approver 是准入责任角色。
> 2026-08-13 更新：CL-01–CL-12 保留为对应产品节点的技术证据。产品顺序和功能完成状态分别以[《游戏引擎产品落地开发计划》](89-engine-product-delivery-plan.md)和[《M1 需求与验收追踪矩阵》](90-m1-requirement-traceability.md)为准。
...
| CL-03 | Windows 壳选型满足文件、更新、内存和隔离 | 进行中（[证据契约](65-cl03-windows-shell-evidence-contract.md)；[Spike 01](82-cl03-windows-shell-spike-01.md)–[06](87-cl03-windows-shell-spike-06.md)） | Windows Platform Owner | Architecture + Security | WS-01–WS-18；WIN-L 预算；安全/数据/更新硬门；通过者再评分 | `eadd13a`：双壳 CAS marker 含 PID/nonce/时间；活 owner 不误删；强杀 holder 后各 8 PID 恢复均 1 acquired/7 held/零 residue；PID 创建时间、时钟/掉电未解；全仓 CL-04 门仍红 | 绑定 Windows process creation time，验证 PID 重用/权限拒绝及时钟/损坏矩阵，再做真实 renderer/main WAL 强杀；WS-06/07 均不得通过 |
| CL-04 | Narrative VM 可确定回滚、前进、快进与 Barrier | 进行中（契约已冻结；[Spike 01](68-cl04-vm-kernel-spike-01.md)–[14](81-cl04-vm-conformance-bundle-spike-14.md)） | Runtime Owner | Architecture + QA | 三端 State Hash 0 差异；VM-01–VM-15；10k 生成序列；不可重放副作用必须显式 Barrier | `ecc29a6`：内容寻址 Bundle、机器报告和 0/2/64/70 退出码已冻结；Node 参考宿主 match，故障注入可定位 | 按 CL-03/CL-02 契约接入 Windows/Android 自执行 Observation，不得转发 Node 结果 |
...
该图表示 Claim 本身的证据依赖，不表示产品主线必须按 CL 编号执行。只有当前产品节点需要某项 Claim 时才恢复对应验证；在 CL-01 实体设备未登记前，任何目标设备 Claim 都不能宣布通过。
