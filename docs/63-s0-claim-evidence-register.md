...
| CL-04 | Narrative VM 可确定回滚、前进、快进与 Barrier | 进行中（契约已冻结；[Spike 01](68-cl04-vm-kernel-spike-01.md)–[06](73-cl04-vm-runtime-save-spike-06.md)） | Runtime Owner | Architecture + QA | 三端 State Hash 0 差异；VM-01–VM-15；10k 生成序列；不可重放副作用必须显式 Barrier | `63382f6`：canonical Runtime Save、VM-11/12、History/Effect/Barrier 恢复、损坏/版本/Build/Opcode fail-closed；仅 Node 内存协议，真实存储/迁移/独立审阅待办 | Spike 07 冻结 Skip/Auto 调度与停止边界，并执行 VM-09/10 |
