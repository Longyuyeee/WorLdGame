...
> E4 实施候选（2026-08-21）：Editor Preview 已直接接入正式 Runtime History/Scheduler，提供 Continue、Step Over、Back/Forward 和执行前 Run to Cursor；内部指令 transient、Choice 阻断、调用栈 Step Over、recorded future 与分支 fork 均有正反例。2 files / 14 tests、98 files / 600 tests、串行 storage、production build 与真实浏览器路径已通过；远端 Windows / Node 22 完整门待取得，因此 E4 尚未关闭，详见 [N32-E4 审计](146-n32-e4-preview-debug-controls-audit.md)。
