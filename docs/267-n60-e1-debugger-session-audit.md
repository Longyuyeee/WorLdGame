...
| Windows CI 修正头 | 精确实现头完整门绿色，裁决本机非稳定超时 | run `33486198643` / job `99786727578`，head `5961dbb`，用时 `13m15s`；普通回归 `155 files`，N60 `3 files / 4 tests`，Route P95 `139.54ms <500ms`，Dicing `3417ms <5000ms`，构建与其余门全部 PASS | 通过；本机累计负载差异关闭，文档头仍需独立 CI，不跨 head 复用 |
