...
6. E4 不宣称宿主/磁盘级 lazy loading、P0 过滤、运行路线高亮、10k 局部编辑或 500 ms P95 已完成；角色/变量/覆盖等高级过滤属于 P1。
...
1. 下一切片先补 P0 章节/节点类型/视觉分组过滤，再单独设计宿主/磁盘级按需读取；不能把当前“全图编译后 64 节点查询”继续称为存储 lazy loading；
