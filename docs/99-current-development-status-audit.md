...
> 最新节点证据：[N32→N40 治理检查点](152-n32-n40-governance-checkpoint.md)、[N40-E1 Route Graph 审计](153-n40-e1-route-graph-core-audit.md)、[N40-E2 10k Route Window 审计](154-n40-e2-10k-route-window-audit.md)、[N40-E3 Layout Sidecar 审计](155-n40-e3-route-layout-sidecar-audit.md)
...
| Story | P0 语言、Writer 卡片、Script、稳定 ID、Compiler IR/Source Map；Route Map 已有 Compiler 事实、Project Service 改名、10k 分支投影、64 节点有界窗口和可保存/重建的最小 Layout Sidecar | N40 分组/折叠/视口/拖拽、存储级按需载入、路线高亮、10k 局部编辑/500 ms 门，N41 完整 Sequence、N60 QA/Debugger |
...
2. N40-E3 已完成最小 Layout Sidecar、剧情隔离与真实工程重开；下一切片进入分组/折叠/视口和交互布局，存储级按需载入、10k 局部编辑和可撤销完整图编辑仍不得提前宣称；
