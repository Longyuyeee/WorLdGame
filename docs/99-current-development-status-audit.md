...
- N40 Route Map Engineering：**E1–E7 已形成 Compiler 图、10k/64 窗口、Layout/过滤、可校验缓存、`<500 ms` 局部编辑和 Runtime 路线高亮；E8a–E8c 已消除重复扫描、建立原子 trusted commit，并让受管工程 verified warm hit 不读取 source-store 正文；E8d 建立 `fullReads=0` 的可信 manifest/chapter/scene 结构索引。结构尚未接入 Route/编辑 Session，script/layout 补页和 production browser 待完成**；N40 Product Acceptance、N41、M1 Stable、Public Release：**被阻断**；
- M1 纵向验收：**0/27 完整通过**；
...
| Story | P0 语言、Writer/Script、Compiler IR/Source Map；Route 有 10k/64 窗口、Layout/过滤、可校验缓存、`<500 ms` 局部编辑及 Formal Runtime 路线高亮；受管工程具备原子 trusted commit、warm hit 无 source-store 正文读取，并可单独可信读取 manifest/chapter/scene 结构 | 结构索引尚未驱动 Route；cache v2 仍恢复完整 Canonical snapshot；缺 scene script/layout 补页、production browser、N41/N60；角色/变量/覆盖过滤属 P1 |
...
2. E6e 已关闭 10k 局部编辑/500 ms P95 子门，E7 已关闭 Runtime 路线高亮，E8a–E8c 已关闭重复扫描、可信 commit 与 warm hit source-store 正文读取，E8d 已冻结可信结构索引；下一步让 Route 首屏消费结构与 Compiler 图，再按窗口/选中 scene 补读 layout/script，并冻结 dirty page、Undo/Redo、保存与冲突边界。接入前不能宣称 lazy loading 完成；安全校验恢复时补做 E5/E6d/E6e/E7/E8a–E8d production browser；
