...
- N52-E3a v2 元数据与截图已有本地实现候选：Store 2.0/DB2 严格兼容 v1 并在下次成功保存 copy-on-write；chapter/scene 只取 Canonical + cursor，route/custom fail closed；12 个手动槽每页 6 个且覆盖二次确认；Host compositor 提供的合规 PNG/WebP 与元数据同事务写入，可见槽位延迟读 Blob，捕获失败仍提交明确无预览的有效 Session。Web Host 不伪造截图；完整本地门与同头 CI 尚待补证，E3b 与 Product Acceptance 未开始；
- E3a 提交前复审补回入口要求的 Preview SHA-256 写前/读取校验与 Blob 篡改反例；最终本地完整门单次通过：普通 `150/905`、N52 `58/58`、VM `24.48s <90s`、Runtime corpus `7.410s`、Route P95 `53.92ms <500ms`、Asset dicing `1624.46ms <5000ms`、17 workspace build 与 architecture `100/4`。仍待实现头推送和同头 Windows CI，不能提前关闭 Engineering；
...
| Player/Build | N50 portable Core/媒体/输入/lifecycle/embed；N51 Settings；N52-E1 History；N52-E2 Save/Load；N52-E3a v2 元数据与截图候选、12 槽分页、Host capture、独立 Blob Store | E3a 同头 CI、真实 Windows/Android compositor、自动/快速/检查点/恢复/Museum、History 页面、Auto/Skip、Gallery、正式宿主与发布材料均缺 |
