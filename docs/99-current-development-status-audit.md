# 当前开发情况审计（N52-E5b Runtime History v2 本地完成，等待 CI）

> 审计日期：2026-09-01
> 当前分支：`codex/n52-e5a-history-contract-authority`；直接基线为 N52 总出口治理最终头 `ec5daa6`
...
> 最新节点证据：[N52-E5b Runtime History v2](260-n52-e5b-runtime-history-v2-audit.md)、[N52-E5a History 入口](259-n52-e5a-history-contract-authority-audit.md)、[N52 总出口治理 checkpoint](258-n52-engineering-exit-and-n60-governance-checkpoint.md)
> 权威功能状态：[M1 需求与验收追踪矩阵](90-m1-requirement-traceability.md)

## 1. 当前结论

2026-09-01 最新纠偏结论是：**N52-E5b 已按实际代码完成 Runtime History/Session Save v2：只读旧分支摘要、active+archive 总界限、活动链唯一导航、v1 先验 hash 后归一及 v2-only write；Player Save v3/DB3、Runtime State/IR/Scheduler 未升版。公共 API 的 `V1` 是代际名而不是磁盘 schema，已保留入口并新增显式 V2/Legacy 类型，避免误伤 Editor/Player。** 本地定向、10k corpus 与 typecheck 已通过，当前等待精确提交 Windows CI；绿色后下一唯一代码切片才是 **N52-E5c Settings + Player Core**。N52 总出口、AC-16、Product Acceptance 与 N60 仍阻断。
...
- N40/N41/N42/N43/N50 Engineering：**出口已通过**；**N51 Engineering 已关闭**；N52 Engineering：**E1–E4f、E5a 已关闭，E5b 本地完成待精确头 CI，E4 出口通过，但 N52 总出口因 Core/Shell History/Barrier 原始需求缺口仍失败**；下一步须先关闭 E5b CI，再进入 E5c；全部 Product Acceptance、N60+、M1 Stable、Public Release：**被阻断**；
- **N52-E5b Runtime History v2 本地实现完成**：[审计 #260](260-n52-e5b-runtime-history-v2-audit.md)证明旧 Forward 已由 Runtime 保存为确定性、只读、不可导航且含完整要求证据的摘要；10,000 总界限 fail closed。Session Save v2 新写、v1/v2 双读，v1 保留旧 artifact identity。Runtime `63/63`、Formal Preview `15/15`、10k seeds/20k replay 零失败，v2 digest `01556a8c…63a9`，State/Outcome 金标不变。当前只缺精确提交 Windows CI，不得提前关闭 E5b 或开始 E5c；
...
| Player/Build | N50 portable Core/媒体/输入/lifecycle/embed；N51 Settings；N52 已完成 Back/Forward、Save/Load、v2/v3 元数据/截图、auto 5、quick 1、隔离恢复、checkpoint 三槽，以及 Auto/Skip/Stop Point/video Web 工程链 | 旧分支归档、可选行 History 页面、Barrier 原因/距离、History Forward 项目策略、真实 Windows/Android compositor、真实强杀、Gallery、正式宿主与发布材料仍缺 |
...
7. N52-E4f 已关闭 Auto/Skip 的 E4 Engineering；总出口治理 #258 发现 History/Barrier 四项缺口，E5a #259 已冻结跨层权限与兼容合同，E5b #260 已在本地实现 Runtime History v2。先取得 E5b 精确头 Windows CI；绿色后下一唯一代码切片为 **N52-E5c Gal Settings v6 + Player Core History projection/navigation**。完成 E5c–E5e 并重新复审总出口前不得进入 N60，全部 Product Acceptance 阻断不变。
