...
> 权威边界：本文件是审计快照；节点状态仍以 [M1 需求与验收追踪矩阵](90-m1-requirement-traceability.md)为准。2026-08-14 后续批准的 [RA-N21-001](100-n21-human-validation-risk-acceptance.md)不修改 N21 未通过事实；[M1 N21 指定集成基线](101-m1-n21-integration-baseline.md)成为 Authoritative 后才关闭 G2。
...
- 当前执行位置：**N20 已通过；N21 工程门通过但真人产品门待完成；`RA-N21-001` 已移除 N22 的真人前置阻断，但仍须先建立指定权威集成基线**；
- GitHub 是否已集成：**尚未；N00–N21 与例外仍未进入 `main`，正在建立一个包含完整祖先链的指定集成分支和整合 PR**。
...
| N22 | 条件准入，尚未实现 | `RA-N21-001` 已接受真人证据延期 | 仍须建立指定权威集成基线；目标为真实资源 Stage/媒体预览，不得关闭 N21 或通过 N23 |
...
- 按 #23 → #30 的依赖顺序评审和合并，或经维护者批准后合并为一个可审查的集成 PR；
- 每一步合并后重跑完整工程门，确认下一个 PR 的基线和差异仍准确；
- 合并完成后关闭或标记被替代的 Draft PR，确保 `main`/指定集成分支成为唯一权威；
- 本审计只记录风险，不擅自替维护者合并 Draft PR。
...
3. N21 人类创作门保持未完成；Draft PR 交付栈只在指定集成基线成为 Authoritative 后关闭 G2，不能因本地分支存在而提前关闭；
