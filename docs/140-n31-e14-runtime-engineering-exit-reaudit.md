...
本机使用 Node `v25.2.1`，符合根 `engines >=22.12.0`，但不是冻结的权威 CI 版本；历史 Spike/正式 seed 性能存在明显波动。同一正式 Runtime 文件在 E13 隔离运行曾以 86.18 秒通过，本次完整 check 为 107.871 秒。该差异没有通过修改 seed 数、90 秒阈值或跳过测试掩盖。权威裁决环境固定 Windows / Node `22.12.0`；E14 独立远端完整门现已绿色，因此登记 N31 Engineering 通过，同时保留本机红项作为环境性能差异证据。
