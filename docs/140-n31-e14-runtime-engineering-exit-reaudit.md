...
> 当前判定：**N31 Engineering 出口候选通过，等待 E14 独立 Windows / Node 22 完整门确认**；N31 Product Acceptance、N32、M1 Stable 与发布继续阻断。
...
这只表示 `@world-studio/runtime` 的 N31 工程契约形成受支持候选。Editor 仍未通过 N32 接入正式 Compiler/Runtime，正式 Player、媒体宿主、存档槽、Windows/Android 产品壳和设备证据均未完成。`RA-N21-003` 明确不授权进入 N32。
...
| 计划项 | 判定 | 证据 |
|---|---|---|
| Goal：Spike 收敛为受支持 Runtime 包 | 候选通过 | portable `@world-studio/runtime` `0.12.0-n31`，不依赖 Spike/Editor/平台 API |
...
| Tests | 候选通过 | 正式 57 项、10k seeds、10k steps、损坏 Save、竞态、Barrier、分支、Node/Worker Golden |
| Acceptance | 候选通过 | 真实浏览器完整门和 E13 最终 Windows / Node 22 全仓门绿色；E14 独立远端门待取得 |
...
| E14 独立支持版本门 | 审计提交的完整仓库门通过 | 尚未创建 PR | 待取得 |

本机使用不受支持的 Node `v25.2.1`，历史 Spike/正式 seed 性能存在明显波动；同一正式 Runtime 文件在 E13 隔离运行曾以 86.18 秒通过，本次完整 check 为 107.871 秒。该差异不能通过修改 seed 数、90 秒阈值或跳过测试修正。权威环境固定 Windows / Node `22.12.0`；E14 只有取得独立远端绿色后才能把候选改为最终 Engineering 通过。

## 5. 需求、产品与授权边界

- REQ-RUNTIME 的 N31 portable kernel 工程前置形成候选；REQ 仍为“实现中”，因为 N32/N50/N52 的 Editor/Player/媒体/玩家槽未完成；
...
1. 推送 E14 独立审计分支并建立 Draft PR；
2. 取得 E14 Windows / Node 22 `product-baseline` 绿色并回写 run/job/耗时；
3. 绿色后登记 N31 Engineering 通过，同时保持 Product/N32/M1/发布阻断；
4. 未获得新的风险接受或完成人工门之前，不进入 N32。
