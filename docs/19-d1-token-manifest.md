...
> 状态：Phase 0 已批准的视觉输入；2026-08-11 起停止 Figma 写入。
> 边界：命名、颜色、间距和动效值继续作为代码设计系统输入，活动实现位于 `apps/editor/src/styles/`；Figma 数量与恢复步骤只保留为历史审计记录，不再执行。
...
以下是停止 Figma 前的恢复计划，仅用于审计当时断点，不再执行：
