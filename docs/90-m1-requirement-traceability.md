...
> 集成边界：当前开发链仍未进入 `main`；N31 集中基线为 Authoritative。N32-E1–E4 已在 Draft PR #52–#55 通过未集成 Engineering；N32-E5 已在 Draft PR #56 run `32467211148` / job `96726246321` 用时 4 分 9 秒并通过完整门，记录为未集成 Engineering 通过。`RA-N21-004` 持续阻断 N32 Product Acceptance、N40 及以后、M1 Stable 与发布。
...
| REQ-QA | 任意入口运行、断点/单步、状态检查、结构错误、循环、源码跳转 | N30/N60 | N31/N32/N40 | 实现中 | Compiler/Runtime 已有 CFG/SCC、Source Map 与结构化诊断；N32-E3/E4 已支持 Fresh Run 与调试控制，E5 可观察并处理 Effect/Barrier/History Host 边界。仍缺断点管理、Watch、Solver 和完整 Debugger E2E | [N30-E2 审计](124-n30-e2-compiler-completion-audit.md)、[N31-E8 审计](133-n31-e8-runtime-source-map-diagnostics-audit.md)、[N32-E3](145-n32-e3-run-from-target-audit.md)–[N32-E5](147-n32-e5-preview-effect-host-audit.md)、QA Golden、Debugger E2E |
