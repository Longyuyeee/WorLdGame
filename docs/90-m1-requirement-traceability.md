...
最近通过节点：`N12`。`N13` 的章节/场景/角色/变量管理、引用迁移、实体搜索和真实 Workspace 保存重开代码已完成本地针对性门，正在等待 Draft PR Windows CI；证据见[《N13 章节、场景、角色和变量管理审计》](96-n13-entity-management.md)。终审后下一节点为 `N20`。
...
|---|---|---|---|---|---|---|
| REQ-PRJ | 新建/打开/最近/示例、保存恢复、章节场景、搜索、桌面/手机工作区、导入导出、离线 | N10–N13/N90/N91 | N00 | 实现中 | N10–N12 已通过；N13 管理与保存重开本地门完成；仍缺 Android SAF、正式壳与强杀恢复 | Project E2E、Recovery Golden、双端任务 |
