...
|---|---|---|---|---|---|
| USP-01 | One Story, Many Views | N40–N43 | 实现中 | N41 Engineering 出口已通过：Sequence/Script/Compiler/Route/IndexedDB 重开同源，1,000 次全 P0 互改不漂移，Formal Runtime statement 已投射到 Sequence；N42-E1–E9 已让 Stage placement、Move easing、关键帧、路径、Camera、转场与文本模板继续使用同一 stable-ID Script；完整时间写入、Debugger 与七模式仍未完成 | 任意工程四视图+Preview+Debugger 同源 E2E |
...
| REQ-STAGE | 画布、安全区、变换、模板、多轨、关键帧、缓动、三视图同步 | N22/N42 | N20/N31 | 实现中 | E1–E8 已关闭真实媒体 Stage、关键帧/时间尺、折线路径、基础 Camera、作用域转场与文本模板；E9 新增单命令三次贝塞尔路径，图形控制点、Inspector、Story Language、Compiler、Runtime/Host、Canvas/DOM Preview 与保存重开共享同一 Move，10,000 条规划为 `5.34ms <500ms`。高级镜头、独立时间写入和正式 Player 一致性仍缺；E9 正等待 Draft PR #72 Windows 完整门复验 | [N42-E9](199-n42-e9-bezier-character-path-audit.md)、[N42-E8](198-n42-e8-dialogue-presentation-templates-audit.md)、AC-13 |
...
| AC-03 | Route/Sequence/Script/Stage 同源 | N40–N43 | 实现中 | E1b–E8 已证明 Stage、关键帧、时间尺、折线路径、Camera、转场和文本模板均由 stable-ID Script 驱动；E9 的三次贝塞尔也只保存一条 canonical Move，Compiler、Runtime/Host、Canvas/DOM 与重开从同一事实重建，不新增路径数据库。完整时间写入和 N43 七模式仍未关闭 | [N42-E9](199-n42-e9-bezier-character-path-audit.md)、[N42-E8](198-n42-e8-dialogue-presentation-templates-audit.md)、ChangeSet/Runtime 对照 |
...
| AC-13 | 镜头/角色/转场/BGM/Voice/SFX | N42/N50 | 实现中 | E2–E8 已让真实角色 Move、关键帧、时间尺、折线路径、基础 Camera、基础转场和文本模板进入正式链；E9 关闭单命令三次贝塞尔角色路径的 Canonical→Compiler→Runtime/Host→Canvas/DOM Preview→重开闭环。镜头高级效果、自定义 shader、真实 Player 渲染/音频 Adapter 与三端媒体宿主仍缺 | [N42-E9](199-n42-e9-bezier-character-path-audit.md)、[N42-E7](197-n42-e7-scoped-stage-transitions-audit.md)、[N42-E6](196-n42-e6-basic-camera-audit.md)、Media Golden 三端快照 |
