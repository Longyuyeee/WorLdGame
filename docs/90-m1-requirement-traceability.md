...
|---|---|---|---|---|---|
| USP-01 | One Story, Many Views | N40–N43 | 实现中 | N41 Engineering 出口已通过：Sequence/Script/Compiler/Route/IndexedDB 重开同源；N42 Stage 能力继续使用同一 stable-ID Script；N43-E1 又证明 Writer/Director/Flow/Quick Start 切换保持同一对白与 `r0`，七模式不建立第二份语义。完整时间写入、Debugger、保存重开上下文协议和其余三模式仍未完成 | [N43-E1](202-n43-e1-workspace-mode-foundation-audit.md)、任意工程四视图+Preview+Debugger 同源 E2E |
...
| REQ-UX | 设计 Token、七模式、Beginner/Pro、统一语义、连续动效、60 FPS、减少动效、多模态状态 | N43/N101 | N21/N40–42 | 实现中 | N43-E1 已冻结七模式身份，真实开放 Writer/Director/Flow/Quick Start，并明确禁用其余三种；桌面/390px Chromium 修正了 16:9 舞台 flex 压缩和固定导航重叠，console error/warn 0。Beginner/Pro、统一上下文、完整七模式任务、减少动效/帧时间和真人证据仍未完成，N21 `0/1`、N23 `0/2` | [N43-E1](202-n43-e1-workspace-mode-foundation-audit.md)、[N42-E3b](193-n42-e3b-preview-production-loop-audit.md)、[N21 执行包](114-n21-human-validation-execution-kit.md)、D1 任务报告 |
...
| AC-11 | 七模式修改同一语句 | N43 | 实现中 | N43-E1 已冻结 7 个身份并开放 4 个；Writer/Director/Flow/Quick Start 切换保持同一对白与 Revision。Production、Debug & QA、Mobile Focus 仍禁用，且尚未证明七模式均可修改/保存重开 | [N43-E1](202-n43-e1-workspace-mode-foundation-audit.md)、七模式 E2E |
