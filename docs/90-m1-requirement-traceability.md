...
> 集成边界：当前开发链仍未进入 `main`；N00–N41 Authority 对应 main-target Draft PR #61，仍等待维护者审阅与合并。N40–N51 Engineering 出口不得换算成 Product Acceptance；**RA-N21-011 的 checkpoint、Stop Point 与 History 窄范围修订**持续阻断 N52 Product Acceptance、N60 及以后、M1 Stable 与发布。N52 E1–E4f、E5a、E5b、E5c Settings/Core Engineering 已关闭；N52 总出口仍因 E5d Shell 与真实桌面/390×844 用户闭环缺口失败。下一实现继续按 #262 完成同一个 Player History 用户闭环，不能直接进入 N60。
...
| USP-09 | Skip / History / Back | N31/N52 | 实现中 | E4 Engineering `7/7` 已关闭 Auto/Skip；总出口复审发现可选行 History 页面、旧 Forward 分支可查看归档、Barrier 原因/距离和 Forward 项目策略仍缺，此外三端正式宿主、实体设备、真人与 Product Acceptance 未完成 | [N52 总出口 checkpoint](258-n52-engineering-exit-and-n60-governance-checkpoint.md)、三端状态一致 |
...
| REQ-RUNTIME | 确定执行、剧情/媒体、Save/History/Auto/Skip/Back、输入、源码错误 | N31/N32/N50/N52 | N20/N30 | 实现中 | Save/Back/Forward、Auto/Skip、Stop Point 与 video 已接正式 Web Player；但 Runtime 未归档被截断旧分支，Core/Shell 未交付可选行 History、Barrier 导航原因和 Forward 项目策略，Windows/Android 正式宿主、实体设备与真人 Product Acceptance 也仍缺 | [N52 总出口 checkpoint](258-n52-engineering-exit-and-n60-governance-checkpoint.md)、[N31-E6](131-n31-e6-runtime-scheduler-audit.md) |
...
| AC-16 | 每句 Back/Forward 和分支截断 | N31/N52 | 实现中 | N52-E1 已证明 State/Host 精确恢复及改选后截断 active Forward；但 Gal 5.2 要求被截断旧分支仍可在 History 查看、选择某行回退并显示不可逆原因，实际 Runtime/Core/Shell 尚未做到，三端和真人证据也仍缺 | [N52 总出口 checkpoint](258-n52-engineering-exit-and-n60-governance-checkpoint.md)、三端玩家 History E2E |
