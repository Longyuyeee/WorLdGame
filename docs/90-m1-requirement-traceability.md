...
| REQ-ROUTE | 完整自动图、布局、局部加载、诊断、路线高亮、进入 Sequence | N40 | N20/N30 | 实现中 | E1–E7 已完成 Compiler 图、10k 窗口、Layout/交互、缓存、局部编辑与 Runtime 路线高亮；E8a–E8j 建立 trusted Route-first、可保存 Script/Sequence、revision-bound 全局索引和 narration 最小结构闭环；E8k 已让 Route topology 只读取当前 scene/layout 页，100 场景首窗 `[1,1,64,64]`，10k Windows `301.56 ms <500 ms`。当前优先缺口是指定结局路线计算/高亮、诊断定位、节点双击进入 Sequence、目标导航/补全及修改后 Preview 复核；Lazy Project 增量、外部目录与平台工作后置；角色/变量/覆盖过滤是 P1 | [N40-E1](153-n40-e1-route-graph-core-audit.md)–[N40-E7](163-n40-e7-runtime-route-highlight-audit.md)、[N40-E8a](164-n40-e8a-single-project-read-audit.md)–[N40-E8k](174-n40-e8k-trusted-route-topology-page-audit.md)、[功能优先复审](175-n40-function-first-development-reaudit.md)、跨视图 E2E |
...
| AC-17 | 脚本自动生成创作者 Route | N40/N62 | 实现中 | E1–E8j 已建立自动图、窗口、缓存、Runtime 高亮、trusted Route-first、全局索引和 narration 结构事务；E8k 关闭 Route structure/topology scene/layout 分页，100 场景首窗 130 个源文件、10k Windows `301.56 ms <500 ms`，并支持 scene.id 与源文件名不同。指定结局静态路线审阅、诊断定位和 Route 驱动修复闭环仍缺；完整 Lazy Project/增量 topology 后置，N62 玩家自动图未开始 | [N40-E1](153-n40-e1-route-graph-core-audit.md)–[N40-E7](163-n40-e7-runtime-route-highlight-audit.md)、[N40-E8a](164-n40-e8a-single-project-read-audit.md)–[N40-E8k](174-n40-e8k-trusted-route-topology-page-audit.md)、[功能优先复审](175-n40-function-first-development-reaudit.md) |
