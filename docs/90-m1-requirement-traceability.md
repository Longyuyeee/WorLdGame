...
最近通过节点：`N00`；当前节点：`N01` 验收中。N01 已建立 50 项唯一 owner、七类 Golden Seed、统一测试入口/证据索引和 PR 追踪更新门，等待 Draft PR Windows CI；证据见[《N01 需求追踪与 Golden 基线审计》](92-n01-requirement-golden-baseline.md)。N00/N01 是共同工程前置，不改变任何 `REQ-*` 或 `AC-*` 的产品完成状态。
...
| AC-08 | 导出后无账户离线重开 | N12/N90/N91 | 未开始 | 通用工程 I/O | 离线导出导入 E2E |
