...
| REQ-STAGE | 画布、安全区、变换、模板、多轨、关键帧、缓动、三视图同步 | N22/N42 | N20/N31 | 验收中 | N42 Engineering `8/8` 保持通过；N50-E2 又把正式 Effect snapshot 映射为 Player PNG 背景/角色、WAV、Camera/Textbox 与 awaited/cancel 生命周期，桌面 16:9 和 390×844 production-browser 实测通过。仍缺 Editor↔Player 差分 Golden、完整多 channel/Voice/SFX、三端 Adapter 与 Product Acceptance | [N50-E2](214-n50-e2-player-stage-media-presentation-audit.md)、[N42 出口复审](200-n42-stage-engineering-exit-reaudit.md)、AC-13 |
...
| REQ-RUNTIME | 确定执行、剧情/媒体、Save/History/Auto/Skip/Back、输入、源码错误 | N31/N32/N50/N52 | N20/N30 | 实现中 | VM-01–VM-15 为 15/15；N50-E1 建立 portable Player Core，E2 增加正式 active/pending/operation Effect snapshot 与真实媒体 adapter。真实测试发现并修正 Compiler 音频字符串→Runtime boolean/permille 断点，缺资源继续 fail closed。玩家槽、历史/控制 UI、Editor↔Player 差分和三端宿主仍缺 | [N50-E2](214-n50-e2-player-stage-media-presentation-audit.md)、[N50-E1](212-n50-e1-formal-player-core-audit.md)、三宿主 Golden、玩家 E2E |
...
| AC-13 | 镜头/角色/转场/BGM/Voice/SFX | N42/N50 | 验收中 | N50-E2 已在正式 Player production build 实际解码 Media Golden PNG、播放 WAV，并证明 awaited complete 与触控 cancel；桌面/手机截图和 JSON 已持久化。仍缺 Editor↔Player 差分、Voice/SFX/多 channel 全矩阵、三端目标设备和 Product Acceptance，故不登记通过 | [N50-E2](214-n50-e2-player-stage-media-presentation-audit.md)、[N42 出口复审](200-n42-stage-engineering-exit-reaudit.md)、Media Golden 三端快照 |
