...
| REQ-ROUTE | 完整自动图、布局、局部加载、诊断、路线高亮、进入 Sequence | N40 | N20/N30 | 实现中 | E1–E4 已完成 Compiler 图、10k 窗口、Layout/交互；E5 browser 待复验；E6a–E6d 已完成可校验缓存基础及 Launcher/Route 接入；E6e 实测 10k 局部编辑；E7 已完成 Formal Runtime 路线高亮；E8a 消除 Launcher 重复全量扫描并让 future schema manifest-only；E8b 为新受管工程落地原子 trusted source commit、逐文件 SHA-256、冲突拒绝和损坏闭锁。Compiler/Route 尚未消费该 commit，current cache hit 仍完整读源，真正局部正文读取及 production browser 复验仍缺；角色/变量/覆盖过滤是 P1 | [N40-E1](153-n40-e1-route-graph-core-audit.md)–[N40-E7](163-n40-e7-runtime-route-highlight-audit.md)、[N40-E8a](164-n40-e8a-single-project-read-audit.md)、[N40-E8b](165-n40-e8b-atomic-source-commit-audit.md)、跨视图 E2E |
...
| AC-17 | 脚本自动生成创作者 Route | N40/N62 | 实现中 | E1–E7 已证明 Compiler 自动图、10k 窗口、布局、过滤、可校验缓存、`<500 ms` 局部编辑和 Runtime 路线高亮；E8a 已消除 Launcher 重复全量扫描并让 future schema manifest-only；E8b 已建立受管工程 trusted source commit。Compiler 尚未消费该 commit，真正存储级正文局部读取、production browser 和 N62 玩家自动图仍缺 | [N40-E1](153-n40-e1-route-graph-core-audit.md)–[N40-E7](163-n40-e7-runtime-route-highlight-audit.md)、[N40-E8a](164-n40-e8a-single-project-read-audit.md)、[N40-E8b](165-n40-e8b-atomic-source-commit-audit.md) |
