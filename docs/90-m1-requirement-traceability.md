...
> 集成边界：当前 N00–N21 能力仍未进入 `main`；[M1 N21 指定集成基线](101-m1-n21-integration-baseline.md)已在 Draft PR #32 成为 Authoritative。N22 与 N23-E1–E7 工程门已完成，N30-E1/E2 Compiler 为未集成工程退出候选；`RA-N21-002` 不改变 N21/N23/N30 产品验收与 M1 仍被阻断，也不授权 N31。
...
最近按产品顺序通过的节点仍是 `N20`；N22/N23 工程门与 N30 Compiler 工程退出候选不跨越 N21 产品门，不能登记 N23 或 N30 产品通过。N21 与 N23 真人记录分别为 `pending-participant`（0/1）和 `pending-participants`（0/2）。`RA-N21-002` 在 2026-09-14 到期，仅允许推进到 N30，并持续阻断 N21/N23/N30 产品验收、N31、M1 Stable 与发布。
...
| REQ-RUNTIME | 确定执行、剧情/媒体、Save/History/Auto/Skip/Back、输入、源码错误 | N31/N32/N50/N52 | N20/N30 | 实现中 | N30 Compiler 工程候选已完成 IR、Source Map、Catalog、增量缓存与 Profile；Editor/独立 HTML 仍使用 N23 运行器，正式 Runtime State 与共享 Editor/Player 未建立 | [N30-E2 审计](124-n30-e2-compiler-completion-audit.md)、VM Golden、三宿主 State Hash、玩家 E2E |
