...
|---|---|---|---|---|---|
| USP-01 | One Story, Many Views | N40–N43 | 实现中 | N41 Engineering 出口已通过：Sequence/Script/Compiler/Route/IndexedDB 重开同源；N42 Stage 继续使用同一 stable-ID Script；N43-E1b 把 mode/view/scene/statement 保存为单一上下文，E5 又让 Production 直接投影同一 Asset Index/Lifecycle，真实保存重开恢复 `production / media_background / 3/3`。完整时间写入、Debugger 和剩余两模式仍未完成 | [N43-E5](208-n43-e5-production-workspace-audit.md)、[N43-E1b](204-n43-e1b-workspace-context-audit.md)、任意工程七模式+Preview+Debugger 同源 E2E |
...
| USP-06 | Professional Studio | N41–N43/N100 | 实现中 | N43-E1–E4 建立统一上下文、Beginner/Pro、Motion 与输入/同步；E5 开放真实 Production 资源任务，连接文件检查、Asset Index、Dicing 与 Runtime 交付验证，当前 5/7。Debug & QA、Mobile Focus 及 Utage 级本地化/配音批量列仍缺 | [N43-E5](208-n43-e5-production-workspace-audit.md)、七模式真实任务、商业演出、Benchmark Episode |
...
| REQ-STAGE | 画布、安全区、变换、模板、多轨、关键帧、缓动、三视图同步 | N22/N42 | N20/N31 | 验收中 | [N42 Engineering 出口复审](200-n42-stage-engineering-exit-reaudit.md)确认 Goal `1/1`、Implementation `8/8`；E1–E9 已覆盖真实媒体、关键帧/时间尺、折线与三次贝塞尔路径、基础 Camera、作用域转场和文本模板，并以 `16 files / 192 tests` 汇总复验 Canonical→Compiler→Runtime/Host→Preview/重开。正式 Player 不存在，Editor↔Player Acceptance `0/1`，故 Product Acceptance 仍阻断 | [N42 出口复审](200-n42-stage-engineering-exit-reaudit.md)、[N42-E9](199-n42-e9-bezier-character-path-audit.md)、AC-13 |
| REQ-UX | 设计 Token、七模式、Beginner/Pro、统一语义、连续动效、60 FPS、减少动效、多模态状态 | N43/N101 | N21/N40–42 | 实现中 | N43-E1–E4 已关闭四模式、统一上下文、渐进披露、Motion 和输入/同步；E5 增加 Production 桌面表/手机状态卡，390px 按钮 `351×48px`、横溢出 0，当前 5/7。其余两模式、真实 OS/目标设备与真人证据仍未完成 | [N43-E5](208-n43-e5-production-workspace-audit.md)、[N43-E4](207-n43-e4-input-sync-and-exit-audit.md)、[N21 执行包](114-n21-human-validation-execution-kit.md) |
...
| AC-03 | Route/Sequence/Script/Stage 同源 | N40–N43 | 实现中 | N42 Stage/路径/Camera/转场/模板均由 stable-ID Script 驱动；N43-E1b 保存重开后 Selection/Inspector/创作态 Runtime 指向同一 stable-ID，E5 Production 又恢复同一 `media_background` 与 Asset Index。完整时间写入、Debugger 和其余两模式仍未关闭 | [N43-E5](208-n43-e5-production-workspace-audit.md)、[N43-E1b](204-n43-e1b-workspace-context-audit.md)、ChangeSet/Runtime 对照 |
...
| AC-10 | 键盘完整操作、手机替代拖拽 | N21/N43/N91 | 实现中 | E4 冻结七类输入等价；路线键盘/按钮共用 24px 命令，390px 真实按钮 `44×44px`。真实浏览器完成键盘文本输入/Ctrl+S 与移动按钮路径；Android 实体触摸/IME、全纯键盘真人任务仍缺 | [N43-E4](207-n43-e4-input-sync-and-exit-audit.md)、N91 设备与真人任务报告 |
| AC-11 | 七模式修改同一语句 | N43 | 实现中 | N43-E1–E4 已开放 4 个模式并关闭 stable-ID 上下文；E5 开放 Production 对同一工程 Asset Index 的真实写入并实测保存重开，当前 5/7。Production 本步写资源而非语句，Debug & QA、Mobile Focus 仍禁用，因此七模式修改同一语句的完整字面门未通过 | [N43-E5](208-n43-e5-production-workspace-audit.md)、[N43-E2](205-n43-e2-progressive-disclosure-audit.md)、七模式 E2E |
...
| AC-13 | 镜头/角色/转场/BGM/Voice/SFX | N42/N50 | 验收中 | N42 Engineering Goal `1/1`、Implementation `8/8` 已通过并由 `16 files / 192 tests` 汇总复验；真实角色、路径、基础 Camera、转场、文本模板和音频资源已进入正式 Editor 链。正式 Player 渲染/音频 Adapter 与 Editor↔Player Golden 不存在，Acceptance 仍为 `0/1` | [N42 出口复审](200-n42-stage-engineering-exit-reaudit.md)、[N42-E9](199-n42-e9-bezier-character-path-audit.md)、Media Golden 三端快照 |
