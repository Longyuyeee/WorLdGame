...
|---|---|---|---|---|---|
| USP-01 | One Story, Many Views | N40–N43 | 实现中 | N41 Engineering 出口已通过：Sequence/Script/Compiler/Route/IndexedDB 重开同源；N42 Stage 继续使用同一 stable-ID Script；N43-E1a 证明四个可用模式不建立第二份语义，E1b 又把 mode/view/scene/statement 保存为单一上下文，Selection/Inspector/创作态 Runtime 在真实 Chrome 保存重开后零漂移。完整时间写入、Debugger 和其余三模式仍未完成 | [N43-E1b](204-n43-e1b-workspace-context-audit.md)、[N43-E1a](202-n43-e1-workspace-mode-foundation-audit.md)、任意工程四视图+Preview+Debugger 同源 E2E |
...
| USP-06 | Professional Studio | N41–N43/N100 | 实现中 | N43-E1 建立四个可用工作模式与统一上下文；E2 增加 Beginner/Pro；E3 关闭共同 Motion Engineering；E4 冻结七类输入等价并实测跨视图 `26.20ms`。Production、Debug & QA、Mobile Focus 仍无真实任务 | [N43-E4](207-n43-e4-input-sync-and-exit-audit.md)、七模式真实任务、商业演出、Benchmark Episode |
...
| REQ-UX | 设计 Token、七模式、Beginner/Pro、统一语义、连续动效、60 FPS、减少动效、多模态状态 | N43/N101 | N21/N40–42 | 实现中 | N43-E1/E2 已关闭四个可用模式、统一上下文和 Beginner/Pro；E3 三级 P95 `12.30/12.20/6.20ms`；E4 冻结七类键盘/触屏等价并将 390px 路线触控目标从 `32×29` 修至 `44×44px`。其余三模式、真实 OS/目标设备与真人证据仍未完成 | [N43-E4](207-n43-e4-input-sync-and-exit-audit.md)、[N43-E3](206-n43-e3-motion-state-semantics-audit.md)、[N21 执行包](114-n21-human-validation-execution-kit.md) |
...
| AC-03 | Route/Sequence/Script/Stage 同源 | N40–N43 | 实现中 | N42 Stage/路径/Camera/转场/模板均由 stable-ID Script 驱动；N43-E1a 模式只改变布局/默认视图，E1b 保存重开后 Selection/Inspector/创作态 Runtime 仍指向同一 stable-ID。完整时间写入、Debugger 和其余三模式仍未关闭 | [N43-E1b](204-n43-e1b-workspace-context-audit.md)、[N42-E9](199-n42-e9-bezier-character-path-audit.md)、ChangeSet/Runtime 对照 |
...
| AC-11 | 七模式修改同一语句 | N43 | 实现中 | N43-E1a 已冻结 7 个身份并开放 4 个；E1b 关闭 stable-ID 保存重开；E2 又证明 Beginner 实际修改同一对白、Pro 恢复、Script 高级上下文保留及 s2 重开零漂移。Production、Debug & QA、Mobile Focus 和七模式修改闭环仍未完成，当前仍是 4/7 | [N43-E2](205-n43-e2-progressive-disclosure-audit.md)、[N43-E1b](204-n43-e1b-workspace-context-audit.md)、七模式 E2E |
