...
最近通过节点：`N13`。`N20` 的 P0 脚本语义、安全表达式、语言服务、通用 Patch 和规模测试已完成本地针对性门，正在等待 Draft PR Windows CI；证据见[《N20 Story Language P0 审计》](97-n20-story-language-p0.md)。终审后下一节点为 `N21`。
...
| REQ-SCRIPT | 高亮/补全/诊断/定义/引用、稳定 ID、格式化/重构、双向同步、外部编辑 | N20/N41 | N10/N11 | 实现中 | N20 语言内核本地门完成；仍缺 N41 正式编辑器呈现、全视图集成和外部编辑 E2E | 100k/round-trip/external edit Golden |
