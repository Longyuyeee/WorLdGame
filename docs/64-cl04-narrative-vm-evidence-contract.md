...
> 决策日期：2026-08-12
> 状态：证据契约已冻结；[Spike 01](68-cl04-vm-kernel-spike-01.md)–[08](75-cl04-vm-meta-progress-spike-08.md) 已完成 VM-01–13 部分基础与 History/Effect/Save/Skip/Meta 前置语义；CL-04 进行中但未通过
> 风险：CL-04
...
现有编辑代码提供编辑事务、Preview 播放按钮、资源调度和项目恢复候选证据；独立 `narrative-vm-spike` 的 [Spike 01](68-cl04-vm-kernel-spike-01.md)–[08](75-cl04-vm-meta-progress-spike-08.md) 已提供 VM-01–13 部分基础、Back/Forward/Fork、受控 Effect/Barrier、canonical Runtime Save、外部 Skip/Auto 与独立 Meta Progress。Save/Meta 仍是单 Node 内存协议，Scheduler 尚未与 Runtime History/真实时钟集成；10k 生成序列和三宿主一致性尚未完成，因此 CL-04 只能标记“进行中”，不得视为正式 VM 或通过。
