...
> 决策日期：2026-08-12
> 状态：证据契约已冻结；[Spike 01](68-cl04-vm-kernel-spike-01.md)–[03](70-cl04-vm-choice-input-spike-03.md) 已完成 VM-01–03 部分基础与 Choice/输入前置语义；CL-04 进行中但未通过
> 风险：CL-04
...
现有编辑代码提供编辑事务、Preview 播放按钮、资源调度和项目恢复候选证据；独立 `narrative-vm-spike` 的 [Spike 01](68-cl04-vm-kernel-spike-01.md)–[03](70-cl04-vm-choice-input-spike-03.md) 已提供 VM-01–03 部分基础、`call/return/random/wait/choice`、显式 execution、确定请求 token、严格外部输入与幂等 receipt。它仍没有 Runtime History/Back/Forward/Fork、Effect Ledger、正式 Save、Skip、10k 生成序列或三宿主一致性执行，因此 CL-04 只能标记“进行中”，不得视为正式 VM 或通过。
