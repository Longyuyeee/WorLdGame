...
| 目标设备 | CL-01 能力档位和预算已冻结，实体设备未登记 | D1 使用逻辑画板；正式证据按 `WIN-L`、`AND-L`、`AND-R` 映射真实设备 |
...
这些是 D1 逻辑画板，不等于最终目标设备；能力档位和预算见[《CL-01 目标设备矩阵与预算冻结》](62-cl01-target-device-budget.md)，正式验证必须记录实体设备指纹。
