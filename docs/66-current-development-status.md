...
| Narrative VM 契约 | 已冻结 State、Step、Effect、Save、History、Barrier、State Hash 与 VM-01–VM-15 | CL-04 进行中；Spike 01–09 完成 VM-01–15 Node 基础及 History/Effect/Save/Skip/Meta/10k 语料前置语义，尚未通过 |
...
| 测试文件 | 54（含独立性能审计文件） |
| 全仓常规测试 | 58 files / 406 tests（`45aad83`） |
| `docs/*.md` | 78（含本快照、CL-02 契约与 CL-04 Spike 01–09 审计） |
...
| Narrative VM | Spike 01–09 已实现 VM-01–15 Node 基础、History、受控 Effect/Barrier、canonical Save、外部 Skip/Auto、独立 Meta 与 10k 固定语料；History+Scheduler、真实时钟/存储、跨版本迁移、三宿主仍未实现 | VM-01–VM-15、10k 生成序列、三宿主 State/Effect/Save Hash 零差异 |
...
| CL-04 Narrative VM | 进行中；契约已冻结 | Spike 01–09 完成 VM-01–15 Node 基础、History/Effect/Barrier、canonical Save、Skip/Auto、独立 Meta Progress 与 10k 固定语料 | Spike 10 冻结可移植语料执行器与逐步 Hash 流，先做 Node/Web Worker 对照 |
...
CL-02、CL-03、CL-04 的证据契约均已冻结，CL-04 Spike 01–09 已在独立堆叠风险分支形成可复现证据。当前代码工作只能继续已冻结契约下的可抛弃 Spike；目标设备未登记前，CL-02/03 只允许环境准备和探索测量，不能继续增加 Web 编辑器功能或宣布任一 Claim 通过。
