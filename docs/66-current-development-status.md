...
| Narrative VM 契约 | 已冻结 State、Step、Effect、Save、History、Barrier、State Hash 与 VM-01–VM-15 | CL-04 进行中；Spike 01–13 完成 10k 语料与 Node/Web Worker 基础、History/Save/Effect/Barrier/Meta/Skip 对照，尚未通过 |
...
| 测试文件 | 54（含独立性能审计文件） |
| 全仓常规测试 | 61 files / 413 tests（`9f56cff`） |
| `docs/*.md` | 82（含本快照、CL-02 契约与 CL-04 Spike 01–13 审计） |
...
| Narrative VM | Spike 01–13 已实现 VM-01–15、10k 固定语料及 Node/Web Worker 的基础、History/Save/Effect/Barrier/Meta/Skip 对照；真实存储、迁移、Windows/Android 壳仍未实现 | VM-01–VM-15、10k 生成序列、三宿主 State/Effect/Save Hash 零差异 |
...
| CL-04 Narrative VM | 进行中；契约已冻结 | Spike 01–13 完成 10k 语料及 Node/Web Worker 的 50 条基础/History/Save/Effect/Barrier/Meta/Skip 记录 | Spike 14 冻结平台壳可执行 Conformance Bundle 与机器可读差异报告 |
...
CL-02、CL-03、CL-04 的证据契约均已冻结，CL-04 Spike 01–13 已在独立堆叠风险分支形成可复现证据。当前代码工作只能继续已冻结契约下的可抛弃 Spike；目标设备未登记前，CL-02/03 只允许环境准备和探索测量，不能继续增加 Web 编辑器功能或宣布任一 Claim 通过。
