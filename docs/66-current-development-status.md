...
| Windows 壳契约 | 已冻结 Electron/Tauri 同切片、WS-01–WS-18、安全、WAL、更新、签名与 WIN-L 比较门；探索 Spike 01 已完成双壳同源 VM 自执行 | CL-03 仍未选型；项目文件、恢复、更新、签名与 WIN-L 尚未证明 |
...
| TypeScript/TSX/MJS 源文件 | 160 |
| 测试文件 | 64（含独立性能审计文件） |
| 全仓常规测试 | 62 files / 416 tests（`7d5e606`） |
| `docs/*.md` | 84（新增 CL-03 Windows 壳探索 Spike 01 审计） |
...
|---|---|---|
| Windows 完整编辑器 | Electron/Tauri 同源 VM 验证宿主已运行；尚无产品壳且未选型 | 两个候选完成相同项目文件/恢复/安装更新切片，在 WIN-L 通过 CL-03 并形成 ADR |
...
| CL-03 Windows 编辑壳 | 进行中；探索 Spike 01 通过 | 同源 Web 在 Electron/Tauri 自执行 10k Bundle 摘要一致；最小桥、安全拒绝和 release 构建成立 | 扩展到 ProjectFileStore/WAL/恢复/单写者，再做安装更新与 WIN-L；暂不选型 |
...
CL-02、CL-03、CL-04 的证据契约均已冻结，CL-04 Spike 01–14 与 CL-03 探索 Spike 01 已形成可复现证据。当前代码工作只能继续已冻结契约下的可抛弃 Spike；目标设备未登记前，CL-02/03 只允许环境准备和探索测量，不能继续增加 Web 编辑器功能或宣布任一 Claim 通过。
