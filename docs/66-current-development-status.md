...
| Narrative VM 契约 | 已冻结 State、Step、Effect、Save、History、Barrier、State Hash 与 VM-01–VM-15 | CL-04 进行中；Spike 01–08 完成 VM-01–13 部分基础及 History/Effect/Save/Skip/Meta 前置语义，尚未通过 |
...
| 测试文件 | 54（含独立性能审计文件） |
| 全仓常规测试 | 57 files / 402 tests（`1c3bf1e`） |
| `docs/*.md` | 77（含本快照、CL-02 契约与 CL-04 Spike 01–08 审计） |
...
| Narrative VM | Spike 01–08 已实现 VM-01–13 部分基础、History、受控 Effect/Barrier、canonical Save、外部 Skip/Auto 与独立 Meta；VM-14/15、History+Scheduler、真实时钟/存储、跨版本迁移、三宿主仍未实现 | VM-01–VM-15、10k 生成序列、三宿主 State/Effect/Save Hash 零差异 |
...
| CL-04 Narrative VM | 进行中；契约已冻结 | Spike 01–08 完成 VM-01–13 部分基础、History/Effect/Barrier、canonical Save、Skip/Auto 与独立 Meta Progress | Spike 09 执行 VM-14/15、至少 10k 生成序列、批处理让步与变形性质测试 |
...
CL-02、CL-03、CL-04 的证据契约均已冻结，CL-04 Spike 01–08 已在独立堆叠风险分支形成可复现证据。当前代码工作只能继续已冻结契约下的可抛弃 Spike；目标设备未登记前，CL-02/03 只允许环境准备和探索测量，不能继续增加 Web 编辑器功能或宣布任一 Claim 通过。
