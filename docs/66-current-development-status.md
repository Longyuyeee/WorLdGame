...
| Narrative VM 契约 | 已冻结 State、Step、Effect、Save、History、Barrier、State Hash 与 VM-01–VM-15 | CL-04 进行中；Spike 01–05 完成 VM-01–08 部分基础及 Choice/History/Effect 前置语义，尚未通过 |
...
| TypeScript/TSX/MJS 源文件 | 133 |
| 测试文件 | 54（含独立性能审计文件） |
| 全仓常规测试 | 54 files / 373 tests（`40b575a`） |
| `docs/*.md` | 74（含本快照、CL-02 契约与 CL-04 Spike 01–05 审计） |
...
| Narrative VM | Spike 01–05 已实现 VM-01–08 部分基础、输入隔离、完整 checkpoint、Back/Forward/Fork、受控 Effect 完成/取消、补偿与 Barrier 阻断；正式 Save、Skip、真实 Scheduler、三宿主仍未实现 | VM-01–VM-15、10k 生成序列、三宿主 State/Effect Hash 零差异 |
...
| CL-04 Narrative VM | 进行中；契约已冻结 | Spike 01–05 完成 VM-01–08 部分基础、History 分叉、确定 Effect token/hash、严格完成/取消、pure/reversible ledger 与 Barrier 阻断 | Spike 06 冻结 Runtime Save envelope、完整性、版本拒绝与非破坏迁移，并执行 VM-11/12 |
...
CL-02、CL-03、CL-04 的证据契约均已冻结，CL-04 Spike 01–05 已在独立堆叠风险分支形成可复现证据。当前代码工作只能继续已冻结契约下的可抛弃 Spike；目标设备未登记前，CL-02/03 只允许环境准备和探索测量，不能继续增加 Web 编辑器功能或宣布任一 Claim 通过。
