...
export function parseStory(source: string): StoryDocument {
  ...
  for (let lineIndex = 0; lineIndex < lines.length; lineIndex += 1) {
    ...
    } else if(trimmed==="checkpoint"||trimmed.startsWith("checkpoint ")){
      const metadata=parseMetadata(trimmed.slice(10));if(metadata.id===undefined||metadata.trailingMetadata!==""||metadata.malformedId)diagnostics.push(diagnostic("MALFORMED_CHECKPOINT","checkpoint requires exactly one @id(...) and no other content",range));node={kind:"checkpoint",...(metadata.id===undefined?{}:{id:metadata.id}),trailingMetadata:metadata.trailingMetadata,range};registerId(metadata.id,range);
