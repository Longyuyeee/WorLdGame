...
export function preflightLazyNarrationStructuralEdit(baseline: ScriptDocument, candidate: ScriptDocument, request: LazyNarrationStructuralRequest): LazyNarrationInsertionPreflight {
  ...
  if (request.kind === "insert-after" || request.kind === "insert-before") {
    ...
    if (candidate.statements.length !== baseline.statements.length + 1 || inserted?.kind !== "narration" || inserted.id !== request.statementId || inserted.textId !== request.textId || typeof inserted.text !== "string" || inserted.text.trim() === "") return fail("UNSUPPORTED_CHANGE", "Candidate is not the declared single non-empty narration insertion");
  ...
  if (baseline.statements[targetIndex]!.kind !== "narration") return fail("UNSUPPORTED_CHANGE", "Only narration statements may be deleted or moved lazily");
