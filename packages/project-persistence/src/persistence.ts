...
export function assertProjectSnapshot(snapshot: ProjectSnapshot): void {
  ...
  for (const scene of snapshot.scenes) {
    ...
    for (const tombstone of scene.tombstones) {
      ...
      } else if (!(["background", "show", "camera", "audio", "textbox"] as const).includes(tombstone.command)) {
...
export async function loadProject(store: ProjectFileStore): Promise<ProjectSnapshot | null> {
  ...
  for (const descriptor of data.scenes) {
    ...
    const tombstones = scene.tombstones.map((item) => {
      ...
      if (item.kind === "directive" && (item.command === "background" || item.command === "show" || item.command === "camera" || item.command === "audio" || item.command === "textbox") && typeof item.argumentsRaw === "string") {
        const command: "background" | "show" | "camera" | "audio" | "textbox" = item.command;
