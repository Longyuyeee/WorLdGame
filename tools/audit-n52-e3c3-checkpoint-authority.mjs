...
if (authority?.status !== "active" || !["N52", "N60"].includes(authority?.maximumDeliveryNode) || authority?.scopeAmendedAt !== contract.authority?.approvedAt || authority?.evidencePath !== "docs/245-n52-e3c3-checkpoint-authority-amendment.md") violations.push("RA-N21-011 checkpoint amendment is not active or exact");
...
for (const gate of ["N52 Product Acceptance", "N60 Product Acceptance", "N61 Engineering", "M1 Stable", "Public Release"]) if (!authority?.blockedGates?.includes(gate)) violations.push(`RA-N21-011 no longer blocks ${gate}`);
