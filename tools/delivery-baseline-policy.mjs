...
export function validateDeliveryBaselineRegistry(registry) {
  ...
  if (registry?.authorityBranch !== "agent/m1-integration-n21") violations.push("authorityBranch must name the frozen N21 integration branch");
  ...
  if (registry?.integratedThrough !== "N21") violations.push("integratedThrough must remain N21");
  ...
  if (JSON.stringify(nodes) !== JSON.stringify(requiredIntegrationNodes)) violations.push("requiredAncestors must preserve the frozen N00-N21 order");
